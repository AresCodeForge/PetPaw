...
export default function OrderReviewPage() {
  ...
  const handlePlaceOrder = async () => {
    ...
    router.push(`/order/confirmation?order_id=${encodeURIComponent(result.order_id)}&order_number=${encodeURIComponent(result.order_number ?? result.order_id?.slice(0, 8) ?? "")}`);
  };
  ...
  if (cart.length === 0 || !shipping) {
    return (
      <div className="mx-auto max-w-xl px-6 py-16">
        ...
        <div className="mt-6">
          <AnimatedButton href="/order" variant="primary">
            {t("order_title")}
          </AnimatedButton>
        </div>
      </div>
  ...
  return (
    <div className="mx-auto max-w-xl px-6 py-16">
      ...
      <div className="mt-8 flex gap-4">
        <AnimatedButton href="/order/checkout" variant="outline">
          {t("common_back")}
        </AnimatedButton>
